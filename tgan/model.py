...
from tensorpack import (
    BatchData, BatchNorm, Dropout, FullyConnected, ModelDescBase, ModelSaver,
...
class GraphBuilder(ModelDescBase):
        ...
        with tf.name_scope("GAN_loss"):
            ...
            with tf.name_scope("discrim"):
                ...
                d_loss = 0.5 * d_loss_pos + 0.5 * d_loss_neg + \
                    slim.apply_regularization(
                        slim.l2_regularizer(l2_norm),
    ...
    def inputs(self):
        ...
        for col_id, col_info in enumerate(self.metadata['details']):
            if col_info['type'] == 'value':
                ...
                inputs.append(
                    tf.TensorSpec(shape = [self.batch_size, 1], dtype=tf.float32,  name = 'input%02dvalue' % col_id))

                inputs.append(
                    tf.TensorSpec(
                        shape = [self.batch_size, gaussian_components], dtype=tf.float32,
                        name = 'input%02dcluster' % col_id
                    )
                )

            elif col_info['type'] == 'category':
                inputs.append(tf.TensorSpec(shape=[self.batch_size, 1], dtype=tf.int32, name= 'input%02d' % col_id))
    ...
    def _get_optimizer(self):
        if self.optimizer == 'AdamOptimizer':
            ...
            return tf.compat.v1.train.AdadeltaOptimizer(self.learning_rate, 0.95)

        else:
            return tf.compat.v1.train.GradientDescentOptimizer(self.learning_rate)
...
class TGANModel:
    ...
    def get_model(self, isTraining=True):
    ...
    def prepare_sampling(self):
        ...
        if self.model is None:
            self.model = self.get_model(isTraining=False)
    ...
    def fit(self, data):
        ...
        self.model = self.get_model(isTraining=True)
